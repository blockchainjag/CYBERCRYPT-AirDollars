...
void Shutdown()
{
    ...
    RenameThread("airdollars-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
#ifdef ENABLE_WALLET
    if (pwalletMain)
        pwalletMain->Flush(false);
    GenerateAirDollarss(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "airdollars.conf"));
    if (mode == HMM_AIRDOLLARSD)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "airdollarsd.pid"));
#endif
    ...
    strUsage += HelpMessageGroup(_("RPC SSL options: (see the AirDollars Wiki for SSL setup instructions)"));
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The AirDollars Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("airdollars-loadblk");
    // -reindex
    if (fReindex) {
        CImportingNow imp;
        int nFile = 0;
...
/** Sanity checks
 *  Ensure that AirDollars is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.airdollars.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AirDollars Core is shutting down."));
...
#endif
    // Make sure only a single AirDollars process is using the data directory.
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AirDollars Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AirDollars Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AirDollars version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AirDollars Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart AirDollars Core to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateAirDollarss(GetBoolArg("-gen", true), pwalletMain, GetArg("-genproclimit", 1));
#endif
