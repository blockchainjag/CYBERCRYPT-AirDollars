...
export AIRDOLLARSCLI=${BUILDDIR}/qa/pull-tester/run-airdollars-cli
export AIRDOLLARSD=${REAL_AIRDOLLARSD}
...
if [ "x${ENABLE_AIRDOLLARSD}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and airdollarsd must all be enabled"
fi
