...
*Setup instructions for a Gitian build of AirDollars using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AirDollars
...
- [Building AirDollars](#building-airdollars)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-airdollars-host/README.md).
...
Clone the git repositories for airdollars and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/airdollars/airdollars
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the airdollars repository
...
To build AirDollars (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the airdollars repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/airdollars/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/airdollars/airdollars
...
For example:
```bash
URL=https://github.com/laanwj/airdollars.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit airdollars=${COMMIT} --url airdollars=${URL} ../airdollars/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit airdollars=${COMMIT} --url airdollars=${URL} ../airdollars/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit airdollars=${COMMIT} --url airdollars=${URL} ../airdollars/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/airdollars-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/airdollars-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/airdollars-osx-build.assert
```
...
[airdollars/gitian.sigs](https://github.com/airdollars/gitian.sigs/) repository, or if that's not possible create a pull
